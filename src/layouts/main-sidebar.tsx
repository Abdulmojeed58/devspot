...
const MainSidebar = () => {
  ...
  return (
    <aside className="w-[220px] bg-dev-card min-h-[calc(100vh-80px)]">
        ...
        <ul className="space-y-[20px]">
          {navItems.map((item) => (
            <li key={item.label}>
              <Link
                href={item.href}
                className={`flex items-center gap-2 px-2 py-1 transition-colors border-l-2 ${
                  pathname === item.href
              ...
              >
                {item.icon}
                <span className="text-sm font-bold leading-5 font-raleway">{item.label}</span>
              </Link>
            </li>
          ))}
        </ul>
